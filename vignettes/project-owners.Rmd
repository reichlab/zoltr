...
- If you specify a different host, you should _always_ use "https" for the URL (it's the "s" that's different from a
  non-secure URL). That is why the default host is "https://zoltardata.com" and not "http://zoltardata.com".
- Zoltar currently uses a five minute timeout on the underlying JWT tokens used under the hood, which means you'll have 
  to re-authenticate after that time.
