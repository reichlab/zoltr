...
Now let's upload a forecast to the model using `upload_forecast()` and then see how to list all of a model's forecasts (in this case just the one). Keep in mind that Zoltar enqueues long operations like forecast uploading, which keeps the site responsive but makes the Zoltar API a little more complicated. Rather than having the `upload_forecast()` function _block_ until the upload is done, you instead get a quick response in the form of a `Job` URL that you can pass to the `job_info()` function to check its status and find out when the upload is pending, done, or failed). (This is called _polling_ the host to ask the status.) Here we poll every second using a helper function:

```{r}
busy_poll_job <- function(zoltar_connection, job_url) {
  cat(paste0("polling for status change. job: ", job_url, "\n"))
  while (TRUE) {
    status <- job_info(zoltar_connection, job_url)$status
    cat(paste0(status, "\n"))
    if (status == "FAILED") {
      cat(paste0("x failed\n"))
      break
...
job_url <- upload_forecast(zoltar_connection, model_url, "2011-10-02", forecast_data)
...
Now that the project has some forecast data, let's query it using the `submit_query()` and `job_data()` functions. Because, like uploading a forecast, querying involves multiple steps: 1) submit the query to receive a job url, 2) poll the job until it finishes, and 3) download the query results as a `data.frame`. Note that we use `list()` here, which will query every forecast in the project. This is will fail for even moderately-sized projects because they could have many thousands or millions of rows. More typically one would constrain the query (TBD). 

```{r}
job_url <- submit_query(zoltar_connection, project_url, list())
busy_poll_job(zoltar_connection, job_url)
```

Like above, hopefully you'll "SUCCESS" eventually printed. Let's download the results:
