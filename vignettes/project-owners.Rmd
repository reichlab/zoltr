...
- Edit your [`.Renviron`](https://csgillespie.github.io/efficientR/3-3-r-startup.html#renviron) file:
    ...
    - `FORECAST_JSON_FILE`: the path of a forecast data file in the format documented
       [here](https://www.zoltardata.com/docs#forecasts)
...
and password for your account. Notes:
...
- Below you will see the host shown as _127.0.0.1:8000_ - this is a temporary one that was used to generate this 
  documentation.
...
Now let's upload the forecast data file FORECAST_JSON_FILE for TIMEZERO_DATE via the `upload_forecast()` function.
...
```{r}
forecast_json_file <- Sys.getenv("FORECAST_JSON_FILE")
forecast_data <- jsonlite::read_json(forecast_json_file)
upload_file_job_id <- upload_forecast(conn, model_id, timezero_date_str, forecast_data)
