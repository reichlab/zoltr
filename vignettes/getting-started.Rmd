...
You can get forecast data using the `download_forecast()` function, which supports a nested `list` or a `data.table`
(tabular) format.

First as a `list`. (Please see [here](https://www.zoltardata.com/docs#forecast-data-format) for forecast format
details.) 

```{r}
first_forecast_id <- the_forecasts[1, 'id']  # assumes at least one exists

forecast_data <- download_forecast(conn, first_forecast_id)
length(forecast_data$predictions)
```

Convert the `list` to a CDC CSV format `data.frame`:

```{r}
forecast_data <- suppressMessages(download_forecast(conn, first_forecast_id))
