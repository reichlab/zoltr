...
## Query a project's score data

Querying scores uses the same functions as that of querying forecasts, along with a very similar query format - all args are the same except `query_type` is `"scores"` and we pass `scores` instead of `types`:

```{r}
score_data <- do_zoltar_query(zoltar_connection, project_url, "scores", "docs forecast model", 
                              c("location1", "location2"), c("pct next week", "cases next week"),
                              c("2011-10-02", "2011-10-09", "2011-10-16"), scores = c("abs_error", "pit"))
score_data
```
...
Similarly, querying truth is done by passing a `query_type` of `"truth"`. Further, only the `xx`, `xx`, and `xx` args are allowed:

```{r}
truth_data <- do_zoltar_query(zoltar_connection, project_url, "truth", NULL, c("location1", "location2"),
