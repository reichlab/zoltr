...
> Note: `submit_query()`'s `query` argument contains IDs (integers) that unambiguously identify the items of interest on the server (models, units, targets, and timezeros). However, dealing with IDs is inconvenient so we provide the `query_with_ids()` helper function, which converts string names to IDs for you. The result can be passed to `submit_query()` as demonstrated in this example.

> Note: You may find the `do_zoltar_query()` function helpful, which combines `query_with_ids()`, `submit_query()`, `busy_poll_job()`, and `job_data()` in one call.

Putting it together, we show the long way to do it (for reference) but use `do_zoltar_query()` to actually run the example:

<!-- show but don't run-->
```{r, eval=FALSE, include=TRUE}
query <- list("targets" = list("pct next week", "cases next week"), "types" = list("point"))
query <- query_with_ids(zoltar_connection, project_url, query)
job_url <- submit_query(zoltar_connection, project_url, query)
...
```{r}
forecast_data <- do_zoltar_query(zoltar_connection, project_url, c("docs forecast model"), 
                                 c("location1", "location2"), c("pct next week", "cases next week"),
                                 c("2011-10-02", "2011-10-09", "2011-10-16"), c("point", "quantile"))
str(forecast_data)
```

Hopefully you'll "SUCCESS" eventually printed and then the resulting data itself.
