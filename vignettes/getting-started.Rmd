...
job_url <- submit_query(zoltar_connection, project_url, TRUE, query)
...
```{r}
forecast_data <- do_zoltar_query(zoltar_connection, project_url, TRUE, "docs forecast model", 
...
Querying scores uses the same functions as that of querying forecasts, along with a very similar query format - all args are the same except `is_forecast_query` is `FALSE` and we pass `scores` instead of `types`:

```{r}
score_data <- do_zoltar_query(zoltar_connection, project_url, FALSE, "docs forecast model", 
                              c("location1", "location2"), c("pct next week", "cases next week"),
