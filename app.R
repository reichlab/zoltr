#
# An example app demonstrating the ZoltR package.
#
...
cond <-
sapply(the_projects, function(project)
name(project) == "public project")
project <- if (any(cond))
the_projects[cond][[1]]
else
NULL
...
cond <-
sapply(the_models, function(project)
name(project) == "Test ForecastModel1")
model <- if (any(cond))
the_models[cond][[1]]
else
NULL
...
cond <-
sapply(the_forecasts, function(forecast)
timezero_date(forecast) == the_timezero_date)
existing_forecast <-
if (any(cond))
the_forecasts[cond][[1]]
else
NULL
if (!is.null(existing_forecast)) {
    cat(
    paste0(
    "- deleting existing forecast",
    the_timezero_date,
    ", ",
    existing_forecast$uri,
    "\n"
    )
    )
...
forecast_csv_file <-
"/Users/cornell/IdeaProjects/forecast-repository/forecast_app/tests/EW1-KoTsarima-2017-01-17-small.csv"
cat(paste0("* uploading file: '", forecast_csv_file, "'\n"))
upload_file_job <-
upload_forecast(model, the_timezero_date, forecast_csv_file)
