...
test_that("delete_project() calls delete_resource", {
  zoltar_connection <- new_connection("http://example.com")
  m <- mock()
  local_mocked_bindings(delete_resource = m)
...
test_that("submit_query() creates a Job", {
  ...
  local_mocked_bindings(content = function(...) {
    jsonlite::read_json("data/job-2.json")
  })

  # test correct url and args passed
  zoltar_connection <- new_connection("http://example.com")
  job_url <- submit_query(zoltar_connection, "http://example.com/api/project/1/", "forecasts", list())
